#!/bin/bash
#
# benchmark.sh - Interactive & CLI Benchmark Script untuk Arch Linux
# Ditargetkan untuk Arch Linux, Manjaro, EndeavourOS (x86_64, ARM64)
# Integrasi pacman package manager, sysbench, openssl, lm-sensors.
#
# Fitur:
#   1) Informasi Sistem Komprehensif (OS Version, Rolling Release Kernel, CPU Freq, Thermal, RAM, Storage, Network)
...
#   6) CPU Stress Test (Single & Multi-Core) dengan Thermal Protection Guard (>85°C)
...
TMPDIR="$(mktemp -d /tmp/benchmark_arch.XXXXXX 2>/dev/null || echo /tmp)"
LOG_FILE="$HOME/benchmark_arch_$(date +%Y%m%d_%H%M%S).txt"
TEMP_MAX_LIMIT=85
...
[ -z "${CPU_MODEL:-}" ] && CPU_MODEL="Generic Arch Linux CPU"
...
print_header() {
    echo -e "${BOLD}${GREEN}Arch Linux Benchmark & Stress Test${RESET}"
    echo "Tanggal   : $(date)"
...
# ---------- System Info ----------
run_sysinfo() {
    section "Informasi Sistem Komprehensif (Arch Linux)"
...
run_cpu_benchmark() {
    ...
    if have sysbench; then
        ...
        echo "(Tip: Install sysbench via 'sudo pacman -S sysbench' untuk hasil standar)"
    fi
    ...
    if have openssl; then
        ...
        echo -e "${YELLOW}[openssl] Tidak terinstall (install via 'sudo pacman -S openssl')${RESET}"
    fi
...
export_report() {
    ...
    {
        echo "=========================================================="
        echo " LAPORAN BENCHMARK ARCH LINUX"
        echo " Tanggal : $(date)"
        echo "=========================================================="
        run_full_benchmark
    } > "$LOG_FILE" 2>&1
